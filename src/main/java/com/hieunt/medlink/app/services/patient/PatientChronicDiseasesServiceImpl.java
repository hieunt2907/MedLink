...
public class PatientChronicDiseasesServiceImpl implements PatientChronicDiseasesService {
    ...
    @Override
    public BaseResponse<PatientChronicDiseasesEntity> updateChronicDisease(Long id,
            PatientChronicDiseaseRequest request) {
    ...
    public BaseResponse<Page<PatientChronicDiseasesEntity>> filterMyChronicDiseases(Pageable pageable) {
        ...
        Page<PatientChronicDiseasesEntity> result = chronicDiseasesRepository
                .filterChronicDiseases(patientProfile.getId(), pageable);

        return new BaseResponse<>("filtering chronic diseases successfully", result);
    }

    @Override
    public BaseResponse<Page<PatientChronicDiseasesEntity>> filterChronicDiseases(Long patientProfileId,
            Pageable pageable) {
        Page<PatientChronicDiseasesEntity> result = chronicDiseasesRepository
                .filterChronicDiseases(patientProfileId, pageable);
    ...
    public BaseResponse<PatientChronicDiseasesEntity> getChronicDisease(Long id) {
        PatientChronicDiseasesEntity entity = chronicDiseasesRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Chronic Disease not found"));

...
public class AdminUserController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<BaseResponse<UserResponse>> updateUser(@PathVariable Long id, @RequestBody AdminUserUpdateRequest user) {
        BaseResponse<UserResponse> response = userService.updateUser(id, user);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/")
    public ResponseEntity<BaseResponse<Page<UserResponse>>> filterUsers(@RequestParam(required = false) String keyword, @PageableDefault(size = 20) Pageable pageable) {
